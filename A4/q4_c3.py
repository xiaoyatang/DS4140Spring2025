...
# Assuming the data from each URL is formatted correctly (each row being a data point)
datasets = [np.loadtxt(io.StringIO(requests.get(url).text)) for url in urls]
data = datasets[2]  # Select the third dataset (C3)
...
ks = range(1, 120,5)  # Testing k from 1 to 9
costs = []
